...
Loop: for (int row = 0; row < rows_count; row++) {
    ...
    for (int column1 = 0; column1 < columns_count; column1++) {
        ...
        if (celltext == ExpectedResult) {
            ...
            println('Medical Parole> Decision Review>Pending DR: ' + Columns_row.get(5).getText())

			String MPDR_PendingDR = Columns_row.get(5).getText()
			CustomKeywords.'myPack.WriteExcel.demoKey'(MPDR_PendingDR, 'Medical Parole> Decision Review>Pending DR')
