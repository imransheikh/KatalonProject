...
WebUI.comment('Transcripts>Transcript Management>Pending Job Receipt')

//Pending Job Order
WebUI.selectOptionByLabel(findTestObject('BITS/Transcripts-Transcript Mgmt/select_ctl00BodyuiStatusDDL'), 'Pending Job Receipt', 
    false)
...
if(WebUI.verifyElementVisible(findTestObject('BITS/Transcripts-Transcript Mgmt/span_No Recordings found'),FailureHandling.OPTIONAL)){
	println('No Recordings found.')
	
	String NoRecordingsfound = 'No Recordings found.'
	CustomKeywords.'myPack.WriteExcel.demoKey'(NoRecordingsfound, 'Transcripts>Transcript Management>Status: Pending Job Receipt, Staff: All')
}
...
Loop: for (int row = 0; row < rows_count; row++) {
    ...
    for (int column1 = 0; column1 < columns_count; column1++) {
        ...
        if (celltext == Total_PendingJobOrder) {
            ...
            println('Transcripts>Transcript Management>Status: Pending Job Receipt, Staff: All: ' + Columns_row.get(8).getText())

			String TM_PendingJobReceipt = Columns_row.get(8).getText()
			CustomKeywords.'myPack.WriteExcel.demoKey'(TM_PendingJobReceipt, 'Transcripts>Transcript Management>Status: Pending Job Receipt, Staff: All')
