...
WebUI.comment('Transcripts>Transcript Management>Pending Job Order')

//Pending Job Order
WebUI.selectOptionByLabel(findTestObject('BITS/Transcripts-Transcript Mgmt/select_ctl00BodyuiStatusDDL'), 'Pending Job Order', 
    false)
...
if (WebUI.verifyElementVisible(findTestObject('BITS/Transcripts-Transcript Mgmt/span_No recordings found'), FailureHandling.OPTIONAL)) {
    ...
    CustomKeywords.'myPack.WriteExcel.demoKey'(NoRecordingsfound, 'Transcripts>Transcript Management>Status: Pending Job Order, Staff: All')
}
...
Loop: for (int row = 0; row < rows_count; row++) {
    ...
    for (int column1 = 0; column1 < columns_count; column1++) {
        ...
        if (celltext == Total_PendingJobOrder) {
            ...
            println('Transcripts>Transcript Management>Status: Pending Job Order, Staff: All: ' + Columns_row.get(8).getText())

            String TM_PendingJobOrder = Columns_row.get(8).getText()

            CustomKeywords.'myPack.WriteExcel.demoKey'(TM_PendingJobOrder, 'Transcripts>Transcript Management>Status: Pending Job Order, Staff: All')
